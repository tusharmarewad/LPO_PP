...
@Component({
  selector: 'app-l2-newdraft-create',
  templateUrl: './l2-newdraft-create.component.html',
  styleUrls: ['./l2-newdraft-create.component.css']
})
export class L2NewdraftCreateComponent implements OnInit {
  userId: number = 0;
  agreements: any[] = [];
  drafts: any[] = [];
  selectedDraft: any = {}; // Store selected row data
  ...
  fetchLeads(): void {
    this.leadService.AddHocAgreementJrLawyer(this.userId).subscribe(
      (response) => {
    ...
    this.leadService.AgreementJrLawyer(this.userId).subscribe(
      (response) => {
